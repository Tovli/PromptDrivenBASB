...
7. For each affected compiled note:
   ...
   - bump `source_count`, set `last_ingested_at`, and refresh `summary` if the synthesis changed
...
9. Refresh `vault/index.md` only when a high-value compiled note was created, became a hub, or substantially changed its synthesis.
10. Append a concise entry to `state/decision-log.md`. If routing of a derived note is below `0.80` confidence, add a case to `state/review-queue.md` instead of guessing.
