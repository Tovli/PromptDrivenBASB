...
- Keep `summary`, `route_reason`, and `tags` current when a note materially changes.

# Session Start

Use `AGENTS.md` and `.basb/prompts/01-session-start.md` as the startup dispatcher.

Read this file before any local `state/SOUL.md` overlay. Treat local `state/SOUL.md` as workspace-specific additions or exceptions, not as the canonical package rules.
